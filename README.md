...
- Drag a file or folder onto another sidebar folder to move it. Drop onto any file to move into its containing folder. Drop onto a root-level file or the empty space below the tree to move back to the notebook root. Escape cancels the drag. Moves preserve open drafts and update their save paths; name conflicts and moving folders into themselves are rejected. Move errors appear in the status bar.
...
- Choose **File → Settings…** to independently adjust UI and content font sizes and enable **Save notes automatically**. Content sizing applies to all three views. Autosave writes after one second of inactivity and flushes pending changes before switching notes, creating a note, or closing the window. Changes apply immediately and persist across restarts; **Reset defaults** restores the original settings.
- On reopening, the app restores its view mode, notebook registry, active notebook, last opened file, both font sizes, autosave preference, and window position and size. Preferences are stored in the platform's app-data directory by eframe. Notes are reloaded from disk; use Save to keep edits when autosave is disabled. Existing saved folders migrate into the notebook registry. Unavailable notebooks remain registered; selecting one reports the problem without replacing the current sidebar. Missing folders or files do not block startup.
- When autosave is disabled, switching documents and closing the window prompts to save unsaved changes. Autosave failures keep the note dirty and retry after a short delay.
